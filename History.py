...
def get_weather_features_monthly(lat,lon):
    today = datetime.now().date()
...
def get_monthly_aqi_from_averages(lat: float,lon : float) -> dict:

    monthly_averages = get_weather_features_monthly(lat, lon)
    pollutant_map = {
        "pm2_5": "PM2.5",
        "pm10": "PM10",
        "nitrogen_dioxide": "NO2",
    ...
    all_dates = set()
    for vals in monthly_averages.values():
        all_dates.update(vals.keys())
    ...
    for date in sorted(all_dates):
        pollutants_for_day = {}
        for api_key, aqi_key in pollutant_map.items():
            if api_key in monthly_averages and date in monthly_averages[api_key]:
                pollutants_for_day[aqi_key] = monthly_averages[api_key][date]
        if pollutants_for_day:
            aqi_info = calculate_pollutants_from_indices(lat, lon, pollutants_for_day)
            results[date] = {
                "AQI": aqi_info["overall_aqi"],
...
# res = get_monthly_aqi_from_averages(22.319258159974734,73.21655888438862)
# print(res)
